...
public class GuiTextFieldCustom extends Gui {
	...
	private GuiPedestal parent = null;
	...
	public GuiTextFieldCustom(int componentId, FontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height, GuiPedestal parent) {
	...
	public boolean textboxKeyTyped(char typedChar, int keyCode)
	{
		...
		else
		{
			...
			default:
				...
				if (isCharacterAllowed(typedChar))
				{
					if (this.isEnabled)
					{
	...
	/**
	 * Returns wheter a character may be typed
	 * @param character
	 * @return
	 */
	public boolean isCharacterAllowed(char character) {
		return 
				...
				&& character != 127
				&& (
						Character.isDigit(character)	//Numbers
						|| character == '.'				//Period
						|| character == 14				//Backspace
						|| character == 211				//Delete
						|| character == 203				//Left
						|| character == 205				//Right
						|| character == 12				//negative
					);
	}
...
//				drawRect(this.x, this.y, this.x + this.width, this.y + this.height, -6250336);
				boolean hovered = mouseX >= this.x && mouseY >= this.y && mouseX < this.x + this.width && mouseY < this.y + this.height;
				int k = this.getHoverState(hovered);
				GuiUtils.drawContinuousTexturedBox(BUTTON_TEXTURES, this.x -1, this.y - 1, 0, 46 + k * 20, this.width + 2, this.height + 2, 200, 20, 2, 3, 2, 2, this.zLevel);
			}
	...
	private void drawSelectionBox(int startX, int startY, int endX, int endY)
	{
		...
		if (endX > this.x + this.width) { endX = this.x + this.width; }

		if (startX > this.x + this.width) { startX = this.x + this.width; }
