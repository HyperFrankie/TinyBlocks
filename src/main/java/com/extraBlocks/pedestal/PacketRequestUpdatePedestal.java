...
public class PacketRequestUpdatePedestal implements IMessage {
	...
	public static class Handler implements IMessageHandler<PacketRequestUpdatePedestal, PacketUpdatePedestal> {
		...
		public PacketUpdatePedestal onMessage(PacketRequestUpdatePedestal message, MessageContext ctx) {
			...
			if(Minecraft.getMinecraft().getIntegratedServer().getWorld(message.dimension).getTileEntity(message.pos) instanceof TileEntityPedestal) {
				TileEntityPedestal te = (TileEntityPedestal) Minecraft.getMinecraft().getIntegratedServer().getWorld(message.dimension).getTileEntity(message.pos);
				if (te != null) {
